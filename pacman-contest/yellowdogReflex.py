...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def feature_safe_food_dist(self, gameState, food):
        pos = gameState.getAgentPosition(self.index)
        enemy_infer_pos = self.getEnemyPos(gameState)
        grid = gameState.data.layout
        width = grid.width
        height = grid.height
        x = width / 2 if self.red else width / 2 - 1
        mid_score = util.Counter()
        for i in range(0, height):
            m_pos = (x, i)
            if gameState.hasWall(x, i):
                mid_score[m_pos] = -2
            else:
                min_enemy_dist = min([self.getMazeDistance(e, m_pos) for e in enemy_infer_pos])
                mid_score[m_pos] = -1 + min_enemy_dist
        best_m_pos = max(mid_score.items(), key=lambda p: p[1])[0]
        best_m_dist = self.getMazeDistance(pos, best_m_pos) - 1
        min_food_dist = 99
        max_food_dist = -1
        nf = None
        for f in food:
            d = self.getMazeDistance(best_m_pos, f)
            if d < min_food_dist:
                nf = f
                min_food_dist = d
            max_food_dist = max(max_food_dist, d)
        dist = prim(self, nf, food)
        dist /= len(food)
        return [dist + best_m_dist, min_food_dist + best_m_dist, max_food_dist + best_m_dist]

    def defenderNearPos(self, gameState, pos):
        enemy_infer_pos = self.getEnemyPos(gameState)
        for e_pos in enemy_infer_pos:
            if self.getMazeDistance(e_pos, pos) < 3:
                return True
        return False
    ...
    def getFeatures(self, gameState, action):
        ...
        if len(enemy_capsule) > 0:
            features['capsuleDistance'] = min([self.getMazeDistance(myPos, c) for c in enemy_capsule])
        enemies = [successor.getAgentState(i) for i in self.getOpponents(successor)]
        defenders = [a for a in enemies if not a.isPacman and a.scaredTimer == 0 and a.getPosition() != None]

        if len(defenders) > 0:
            dists = [self.getMazeDistance(myPos, a.getPosition()) for a in defenders]
            features['defenderDistance'] = min(min(dists), 5)
            if not myState.isPacman:
                features['defenderDistance'] = 5
        ...
        if myState.isPacman:
            ...
            food_dist = self.feature_safe_food_dist(successor, fl)

        features['distanceToFood'] = 0.05 * food_dist[0] + 0.95 * food_dist[1]
        if len(enemy_capsule) > 0:
        ...
        for i in range(0, height):
            ...
            if not gameState.hasWall(x, i):
                for d in defenders:
                    if self.getMazeDistance((x, i), d.getPosition()) < 3:
                        b = False
                if b:
        ...
        features['food_placed'] = successor.getAgentState(self.index).numReturned
    ...
    def getWeights(self, gameState, action):
        ...
        defenders = [a for a in enemies if not a.isPacman and myState.isPacman and a.scaredTimer == 0]
        defenders_near = [a for a in enemies if not a.isPacman and a.getPosition() is not None
                          and myState.isPacman and a.scaredTimer < 5]
        defender_timer = 5 if len(defenders_near) == 0 else min(5, min([a.scaredTimer for a in defenders_near]))
        numDefenders = len(defenders)
        numDangerousDefenders = len(defenders_near)
        timeleft = min(50, gameState.data.timeleft)
        if numDefenders > 0:
            dists = [self.getMazeDistance(myState.getPosition(), a.getPosition())
                     for a in defenders if a.getPosition() is not None]
            if len(dists) == 0:
                defenderDistance = 5
            else:
                defenderDistance = min(min(dists), 5)
            if not myState.isPacman:
                defenderDistance = 5
        else:
            defenderDistance = 5
        w = util.Counter()
        w['capsulesLeft'] = -30 * numDangerousDefenders * (5 - defenderDistance) - 10 * numDefenders - 1
        w['capsuleDistance'] = -2 * numDangerousDefenders * (4 - defenderDistance)
        w['successorScore'] = 100
        w['distanceToFood'] = -1
        w['food_carrying'] = -1
        w['food_placed'] = 100
        w['mid_dist'] = -0.1 * numCarrying * numDefenders * numDefenders \
                        - 0.65 * numCarrying * numDangerousDefenders * numDangerousDefenders \
                        - 0.1 * numDefenders - 0.3 * numDangerousDefenders \
                        - 0.05 * numCarrying - 0.05 * (50 - timeleft) - 0.2 * (5 - defender_timer)
        w['stop'] = -3
        w['reverse'] = -2
        w['defenderDistance'] = 20
        w['distanceToCapsule'] = -0.4 * numDefenders - 0.9 * numDangerousDefenders - 0.2 * (5 - defenderDistance)
        return w
...
class DefensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        grid = gameState.data.layout
        width = grid.width
        myborder = width / 2 - 1 if self.red else width / 2
        height = grid.height
        ...
        if len(invaders) > 0:
            ...
            for a in invaders:
                if a[1].getPosition() is None:
                    if a[0] < 2:
                        try:
                            dists.append(self.v_dist_obj.getDistance(myPos, enemy_infer_pos[0]))
                        except:
                            dists.append(self.getMazeDistance(myPos, enemy_infer_pos[0]))

                    else:
                        try:
                            dists.append(self.v_dist_obj.getDistance(myPos, enemy_infer_pos[1]))
                        except:
                ...
                if eid < 2:
                    try:
                        features['mostCarryEnemyDist'] = self.v_dist_obj.getDistance(myPos, enemy_infer_pos[0])
                    except:
                        features['mostCarryEnemyDist'] = self.getMazeDistance(myPos, enemy_infer_pos[0])
                else:
                    try:
                        features['mostCarryEnemyDist'] = self.v_dist_obj.getDistance(myPos, enemy_infer_pos[1])
                    except:
        ...
        if self.red:
            ...
            x = width / 2
        enemy_pos_delta_x = [abs(x - enemy_infer_pos[0][0]) + 0.001, abs(x - enemy_infer_pos[1][0]) + 0.001]
        if len(enemy_pos_delta_x) == 0:
            mid_y = height / 2
        else:
            mid_y = (enemy_infer_pos[0][1] / sum(enemy_pos_delta_x) * (sum(enemy_pos_delta_x) - enemy_pos_delta_x[0])
                     + enemy_infer_pos[1][1] / sum(enemy_pos_delta_x) * (
                             sum(enemy_pos_delta_x) - enemy_pos_delta_x[1])) / 2
        mid_y = int(mid_y)
        for i in range(0, height / 2):
            if 0 < mid_y + i < height and not gameState.hasWall(x, mid_y + i):
                mid_dist = min(mid_dist, self.getMazeDistance(myPos, (x, mid_y + i)))
            if 0 < mid_y - i < height and not gameState.hasWall(x, mid_y - i):
                mid_dist = min(mid_dist, self.getMazeDistance(myPos, (x, mid_y - i)))
        features['mid_dist'] = mid_dist
